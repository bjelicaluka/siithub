...
export const MilestonesPage: FC<MilestonesPageProps> = ({ repo, username }) => {
  ...
  const onDeleteClicked = (milestone: Milestone) => {
    if (confirm(`Are you sure that you want to delete ${milestone.title}?`))
  ...
  return (
    <>
      <div className="px-4 py-3 text-right sm:px-6">
        <Button><Link href={`/${username}/${repo}/milestones/new`}>New milestone</Link></Button>
      </div>
      <div className="flex bg-white border-b dark:bg-gray-800 dark:border-gray-700 p-4">
        <p className={`text-base font-semibold mr-4 ml-4 cursor-pointer ${open ? "text-white" : "text-gray-500"}`} onClick={() => setOpen(true)}>
          Open
        </p>
        <p className={`text-base font-semibold mr-4 cursor-pointer ${open ? "text-gray-500" : "text-white"}`} onClick={() => setOpen(false)}>
          Closed
        </p>
      </div>
      {
        milestones?.map(milestone => 
          <div key={milestone._id} className="grid grid-cols-6 gap-6 bg-white border-b dark:bg-gray-900 dark:border-gray-700 p-4">
            <div className="col-span-2 m-3">
              <Link href={`/${username}/${repo}/milestones/${milestone.localId}`} className="text-2xl font-semibold dark:text-white cursor-pointer hover:text-blue-500">
                {milestone.title}
              </Link>
              <p className="text-base font-semibold dark:text-white">
                {milestone?.dueDate ? `Due by ${new Date(milestone.dueDate).toDateString()}` : "No due date"}
              </p>
              {milestone?.description ? <p className="text-base font-semibold dark:text-white pt-2">{milestone.description}</p> : <></>}
            </div>
            <div className="col-span-4 m-3">
              ...
              <div className="flex">
                <Link href={`/${username}/${repo}/milestones/${milestone.localId}/edit`} className="text-base font-semibold text-blue-400 mr-2">
                  Edit
                </Link>
                <p className="text-base font-semibold text-blue-400 mr-2 cursor-pointer" 
                  onClick={() => (milestone.isOpen ? closeMilestoneAction : openMilestoneAction)(milestone)}>
                  {milestone.isOpen ? "Close" : "Reopen"}
                </p>
                <p className="text-base font-semibold text-red-400 mr-2 cursor-pointer" onClick={() => onDeleteClicked(milestone)}>
                  Delete
                </p>
              </div>
            </div>
