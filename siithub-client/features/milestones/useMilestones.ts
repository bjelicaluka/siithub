...
export function useMilestones(username: string, repo: string, open: boolean, dependencies: any[] = []) {
  const { data, error } = useQuery(
    [`milestones_${username}/${repo}`, open, ...dependencies],
    () => getRepositoryMilestones(username, repo, open),
    {
...
export function useSearchMilestones(username: string, repo: string, title: string, dependencies: any[] = []) {
  const { data, error } = useQuery(
    [`milestones_${username}/${repo}`, title, ...dependencies],
    () => searchRepositoryMilestones(username, repo, title),
    {
      enabled: dependencies.reduce((acc, d) => acc && !d, true),
    }
  );
...
export function useMilestone(username: string, repo: string, localId: number, dependencies: any[] = []) {
  const { data, error } = useQuery(
    [`milestones_${username}/${repo}/${localId}`, ...dependencies],
    () => getMilestone(username, repo, localId),
    {
...
export function useMilestonesByRepoId(repositoryId: Repository["_id"], dependencies: any[] = []) {
  const { data, error } = useQuery(
    [`milestones_${repositoryId}`, ...dependencies],
    () => getMilestonesByRepositoryId(repositoryId),
    {
