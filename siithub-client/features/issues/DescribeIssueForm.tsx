...
export const DescribeIssueForm: FC = () => {
  ...
  useEffect(() => {
    if (!isEdit || initiallySet || !quill) return;
    reset(issue?.csm);
    quill.clipboard.dangerouslyPasteHTML(issue?.csm?.description || '');
    setInitiallySet(true);
  }, [issue.csm.title, issue.csm.description, isEdit, initiallySet, quill]);
