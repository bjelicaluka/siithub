...
export const MilestonesForm: FC = () => {
  ...
  const onMilestonesChange = (milestones: any): void => {
    if (milestones.length > selectedMilestones.length) {
      ...
      issueDispatcher(
        isEdit ? instantAssignMilestoneTo(issue, added.value, executedBy) : assignMilestone(added.value, executedBy)
      );
    } else {
      const removed = findDifference(selectedMilestones, milestones);
      issueDispatcher(
        isEdit
          ? instantUnassignMilestoneFrom(issue, removed.value, executedBy)
          : unassignMilestone(removed.value, executedBy)
      );
    }
